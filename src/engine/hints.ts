...
/** One way to reach a fact: a fully-worked sentence plus an optional visual. */
...
// --- hint string can never disagree with the arithmetic it claims. ---------

/** n×n — an anchor to memorize outright. */
function square(n: number): Strategy {
  return { text: `Square anchor — memorize it: ${n}×${n} = ${n * n}` }
}

/** 10×o via place value. `o` is the non-ten operand. */
function placeValue(o: number): Strategy {
  return { text: `Place value: 10×${o} = ${o}0` }
}

/** 5×o = half of 10×o. */
function halfOfTen(o: number): Strategy {
  return { text: `Half of ×10: 5×${o} = ${10 * o} ÷ 2 = ${5 * o}` }
}

/** 5×o by skip-counting. */
function countByFives(o: number): Strategy {
  return { text: `Count by fives up to ${5 * o}` }
}

/** 11×o for o ≤ 9: the digit doubles. */
function repeatDigit(o: number): Strategy {
  return { text: `Repeat the digit: 11×${o} = ${11 * o}` }
}
 ...
 * Keys are canonical "AxB" with A ≤ B, matching ALL_FACT_KEYS.
 */
...
const FALLBACK: FactHints = {
  primary: { text: 'Break it into smaller, friendlier pieces and add them up.' },
}
